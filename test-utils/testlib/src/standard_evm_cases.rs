...
pub fn test_evm_call_standard_precompiles(node: impl Node) {
    let node_user = node.user();
    let bytes = hex::decode(
        include_bytes!("../../../runtime/near-evm-runner/tests/build/StandardPrecompiles.bin").to_vec(),
    )
    .unwrap();
    ...
    // ECRECOVER
    let (input, _decoder) = precompiles::functions::test_ecrecover::call();
    let args = encode_view_call_function_args(alice_address, contract_id, U256::zero(), input);
    let bytes = node_user
        .function_call(alice_account(), evm_account(), "view", args.clone(), 10u64.pow(14), 0)
        .unwrap()
        ...
        .as_success_decoded()
        .unwrap();
    let res = precompiles::functions::test_ecrecover::decode_output(&bytes).unwrap();
    assert_eq!(res, true);
}
